...
public interface CourseJpaRepository extends JpaRepository<CourseJpaEntity, UUID>, JpaSpecificationExecutor<CourseJpaEntity> {
    ...
    @Query("SELECT c FROM CourseJpaEntity c LEFT JOIN FETCH c.syllabus s WHERE c.id = :id ORDER BY s.order")
    Optional<CourseJpaEntity> findByIdWithSyllabusOrdered(UUID id);
}
